...
export default function Map(props: MapProps) {
    ...
    const row4 = mapData.slice(12,18)
    const row5 = mapData.slice(18,23)
    const row6 = mapData.slice(23,27)
    const row7 = mapData.slice(27,30)
          ...
          <div className="flex justify-start items-center flex-grow-0 flex-shrink-0 md:space-x-[-20px] space-x-[-12px]">
            ...
            <Port type={portData[4].type} position={"3"}></Port>
              ...
              <div className="flex-col justify-start items-start gap-1.5 inline-flex">
                ...
                {
                  row5.map((item, index) => {
                    return (<Tile key={index} tile={item.type} num={item.num.toString()} index={18 + index} onClick={() => tileClick(18 + index)} selected={props.selectedTile === 18 + index}></Tile>)
                  })
                }
              ...
              <div className="flex-col justify-start items-start gap-1.5 inline-flex">
                ...
                {
                  row6.map((item, index) => {
                    return (<Tile key={index} tile={item.type} num={item.num.toString()} index={23 + index} onClick={() => tileClick(23 + index)} selected={props.selectedTile === 23 + index}></Tile>)
                  })
                }
              ...
              <div className="flex-col justify-start items-start gap-1.5 inline-flex">
                ...
                {
                  row7.map((item, index) => {
                    return (<Tile key={index} tile={item.type} num={item.num.toString()} index={27 + index} onClick={() => tileClick(27 + index)} selected={props.selectedTile === 27 + index}></Tile>)
                  })
                }
                <Tile  tile={'Placeholder'} num={"-1"}></Tile>
              </div>
